...
class GracefulShutdown:
    """
    Class providing graceful shutdown capabilities for streaming jobs.
    
    Allows in-progress batches to complete before job termination,
    ...
    def __init__(
        ...
        Args:
            graceful_shutdown_timeout: Maximum seconds to wait for batch completion (default: 30)
    ...
    def wait_for_batch_completion(self) -> bool:
        """
        Wait for current batch to complete within timeout.
        
        Logs progress every shutdown_progress_interval seconds.
        
        Returns:
            True if shutdown was graceful (batch completed), False if timeout exceeded
        """
    ...
    def mark_batch_start(self, batch_id: int) -> None:
        """
        Mark batch as started for tracking.
    ...
    def should_skip_batch(self) -> bool:
        """
        Check if batch should be skipped due to shutdown request.
        
        Returns True if shutdown is requested AND no batch is currently in progress.
        This allows in-progress batches to complete while preventing new ones from starting.
